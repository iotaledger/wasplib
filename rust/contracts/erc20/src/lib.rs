...
// state variable
const STATE_VAR_SUPPLY: &str = "s";       // supply constant
...
fn init(ctx: &ScCallContext) {
    ...
    // validate parameters
    // supply
    let supply = ctx.request().params().get_int(PARAM_SUPPLY);
    ...
    let creator = ctx.request().params().get_agent(PARAM_CREATOR);
    if !creator.exists(){
...
fn balance_of(ctx: &ScViewContext){
    let account = ctx.request().params().get_agent(PARAM_ACCOUNT);
    if !account.exists(){
...
fn allowance(ctx: &ScViewContext){
    ...
    // validate parameters
    // account
    let owner = ctx.request().params().get_agent(PARAM_ACCOUNT);
    if !owner.exists(){
        ctx.log("er20.allowance.fail: wrong 'account' parameter");
        return;
    }
    // delegation
    let delegation = ctx.request().params().get_agent(PARAM_DELEGATION);
...
fn transfer(ctx: &ScCallContext) {
    ...
    let balances = ctx.state().get_key_map(STATE_VAR_BALANCES);
    let source_balance = balances.get_int(ctx.request().sender().to_bytes());
...
fn approve(ctx: &ScCallContext) {
    ...
    // validate parameters
    let account = ctx.request().params().get_agent(PARAM_DELEGATION);
    if !account.exists(){
        ctx.log("erc20.approve.fail: wrong 'delegation' parameter");
        return;
    }
    let account = account.value();
    let amount = ctx.request().params().get_int(PARAM_AMOUNT).value();
    if amount <= 0 {
        ctx.log("erc20.approve.fail: wrong 'amount' parameter");
        return;
    }
    let caller = ctx.request().sender().to_string();
...
fn transfer_from(ctx: &ScCallContext) {
    ...
    let account = account.value();
    let recipient = ctx.request().params().get_agent(PARAM_RECIPIENT);
    if !recipient.exists(){
        ctx.log("erc20.approve.fail: wrong 'recipient' parameter");
        return;
    }
    let recipient = recipient.value();
    let amount = ctx.request().params().get_int(PARAM_AMOUNT);
    if !amount.exists(){
        ctx.log("erc20.approve.fail: wrong 'amount' parameter");
        return;
    }
