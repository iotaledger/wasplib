// Copyright 2020 IOTA Stiftung
// SPDX-License-Identifier: Apache-2.0

// (Re-)generated by schema tool
//////// DO NOT CHANGE THIS FILE! ////////
// Change the json schema instead

#![allow(dead_code)]

use wasmlib::*;

use crate::*;

pub const IDX_PARAM_NUMBER: usize = 0;
pub const IDX_PARAM_PLAY_PERIOD: usize = 1;
pub const IDX_RESULT_LAST_WINNING_NUMBER: usize = 2;
pub const IDX_VAR_BETS: usize = 3;
pub const IDX_VAR_LAST_WINNING_NUMBER: usize = 4;
pub const IDX_VAR_LOCKED_BETS: usize = 5;
pub const IDX_VAR_PLAY_PERIOD: usize = 6;

pub const KEY_MAP_LEN: usize = 7;

pub const KEY_MAP: [&str; KEY_MAP_LEN] = [
    PARAM_NUMBER,
    PARAM_PLAY_PERIOD,
    RESULT_LAST_WINNING_NUMBER,
    VAR_BETS,
    VAR_LAST_WINNING_NUMBER,
    VAR_LOCKED_BETS,
    VAR_PLAY_PERIOD,
];

pub static mut IDX_MAP: [Key32; KEY_MAP_LEN] = [Key32(0); KEY_MAP_LEN];

pub fn idx_map(idx: usize) -> Key32 {
    unsafe {
        IDX_MAP[idx]
    }
}
