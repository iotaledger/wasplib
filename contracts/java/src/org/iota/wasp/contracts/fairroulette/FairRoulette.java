...
public class FairRoulette {
	...
	public static void funcLockBets(ScFuncContext ctx, FuncLockBetsParams params) {
		...
		req.Function = Consts.HFuncPayWinners;
		req.Params = null;
		req.Transfer = null;
		req.Delay = 0;
		ctx.Post(req);
	...
	public static void funcPayWinners(ScFuncContext ctx, FuncPayWinnersParams params) {
		...
		ScMutableMap state = ctx.State();
		state.GetInt(Consts.VarLastWinningNumber).SetValue(winningNumber);
	...
	public static void funcPlaceBet(ScFuncContext ctx, FuncPlaceBetParams params) {
		...
		if (betNr == 0) {
			long playPeriod = state.GetInt(Consts.VarPlayPeriod).Value();
			if (playPeriod < 10) {
				playPeriod = DefaultPlayPeriod;
			}
			PostRequestParams req = new PostRequestParams();
			req.ContractId = ctx.ContractId();
			req.Function = Consts.HFuncLockBets;
			req.Params = null;
			req.Transfer = null;
	...
	public static void funcPlayPeriod(ScFuncContext ctx, FuncPlayPeriodParams params) {
		...
		ctx.State().GetInt(Consts.VarPlayPeriod).SetValue(playPeriod);
	}
}
