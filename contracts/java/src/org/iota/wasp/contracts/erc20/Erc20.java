...
public class Erc20 {
	...
	public static void funcInit(ScFuncContext ctx, FuncInitParams params) {
		...
		ctx.State().GetInt(Consts.VarSupply).SetValue(supply);
		...
		ctx.State().GetMap(Consts.VarBalances).GetInt(creator).SetValue(supply);
	...
	public static void viewAllowance(ScViewContext ctx, ViewAllowanceParams params) {
		...
		long allow = allowances.GetInt(params.Delegation.Value()).Value();
		ctx.Results().GetInt(Consts.ParamAmount).SetValue(allow);
	}
	...
	public static void viewBalanceOf(ScViewContext ctx, ViewBalanceOfParams params) {
		...
		long balance = balances.GetInt(params.Account.Value()).Value();
		ctx.Results().GetInt(Consts.ParamAmount).SetValue(balance);
	}
	...
	public static void viewTotalSupply(ScViewContext ctx, ViewTotalSupplyParams params) {
		long supply = ctx.State().GetInt(Consts.VarSupply).Value();
		ctx.Results().GetInt(Consts.ParamSupply).SetValue(supply);
	}
}
