...
func getString(ctx *client.ScViewContext) {
	...
	s := ctx.State().GetString(client.Key("storedString")).Value()
	// return the string value in the result dictionary
	ctx.Results().GetString(client.Key("paramString")).SetValue(s)
}
